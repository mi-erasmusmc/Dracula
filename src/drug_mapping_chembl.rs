...
pub async fn read_descriptions(pool: &Pool) -> Result<(), Box<dyn Error>> {
    ...
    //         tty.1
    //     );
    let all_chembl = all_chembl(&client, &queries);
...
async fn all_chembl(client: &Object<Manager>, queries: &HashMap<String, String>) {
...
fn do_matching_chembl(
    chembl: Vec<Row>,
    mapping: &Vec<Row>,
    insert_values: &mut String,
    map: &mut HashMap<String, String>,
) {
    let total: i64 = (chembl.len() * mapping.len()) as i64;
    info!("Making {} string comparisons ...", total);
    let mut pb = ProgressBar::new((total / 1000000) as u64);
    pb.set_width(Some(80));
    let mut counter: i64 = 0;
    for c in chembl {
        let drug: String = r.get("synonyms");
        let drug_with_spaces = format!(" {} ", drug);
        for m in mapping {
            counter += 1;
            if counter % 1000000 == 0 {
